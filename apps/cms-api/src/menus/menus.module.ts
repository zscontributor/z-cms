...
class MenusController {
  ...
  async replace(
    ...
    // Once an admin configures a location by hand, the theme's DEMO menu for the
    // same location must yield — otherwise render (and this list) would keep
    // shadowing the hand-made menu with the demo one (see render.service
    // `preferThemeDemo`), and the edit the admin just made would never appear.
    // Deleting it is safe: demo menus are regenerated by "Seed demo content".
    await db().menuItem.deleteMany({
      where: { menu: { siteId, key, demoThemeKey: { not: null } } },
    });
    await db().menu.deleteMany({ where: { siteId, key, demoThemeKey: { not: null } } });
