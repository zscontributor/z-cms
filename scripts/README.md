...
| `release.sh` | `./scripts/release.sh <version> [--yes] [--watch]` | Cuts a Docker release. Picks `origin/main` (or `--ref`), **refuses unless that commit's CI is green** (waits if still running), tags it `v<version>` and pushes — which triggers [`release.yml`](../.github/workflows/release.yml) to build every service image multi-arch (amd64+arm64) and push to **GHCR + Docker Hub** (`zcms/<svc>:<version>` + `<major.minor>` + `latest`) — then creates the GitHub Release. Needs `gh` authenticated and the `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` repo secrets. `--watch` waits for the build and prints the resulting Docker Hub tags. |
